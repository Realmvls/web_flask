...
TARGET = 'mysql+mysqlconnector://root:qwe123,.@140.143.97.62:3306/spider_data?charset=utf8'
...
class mySpider(object):
    ...
    def get_one_html(self,url):
        ...
        for job in jobs:
            ...
            # 公司性质
            gs_xz = job.find_all("li",attrs={"class":"newlist_deatil_two"})[0].find_all("span")[1].text.strip()
            # 公司规模
            gs_gm = job.find_all("li",attrs={"class":"newlist_deatil_two"})[0].find_all("span")[2].text.strip()

            print(data['gs_link'])
            html = requests.get(data['gs_link'], headers=self.headers).text
            print(html)
            soup = BeautifulSoup(html, "html.parser")
           ...
           # xlyq = job.find_all("li",attrs={"class":"newlist_deatil_two"})[0].find_all("span")[3].text.strip()
            try:
                data["gs_xz"] = gs_xz.replace("公司性质：","")
            except:
                data["gs_xz"] = gs_xz
            try:
                data["gs_gm"] = gs_gm.replace("公司规模：","")
            except:
                data["gs_gm"] = gs_gm
            # if "学历" in xlyq:
            ...
            all_data = pd.DataFrame(data,index=range(1))
            all_data.to_sql("jobs",engine,if_exists="append",index=False)
...
if __name__ == '__main__':
    ...
    for i in jobs:
        for j in citys:
            ...
            s.get_one_html('http://sou.zhaopin.com/jobs/searchresult.ashx?jl=%E6%AD%A6%E6%B1%89&kw=python&p=1')
            # print(s)
            # s.main()
